...
def path_counts_query(table_id, from_date, to_date):
    ...
    new_query['filters'] = ','.join([
        ...
        r'ga:pagePath=~^/(articles|reviewed-preprints)/[0-9]+$', # note: GA doesn't support {n,m} syntax ...

        # ga:pagePath=~^/articles/50101/executable$
        r'ga:pagePath=~^/(articles|reviewed-preprints)/[0-9]+/executable$',
    ])
    return new_query
...
REGEX = r"/(?:articles|reviewed-preprints)/(?P<artid>\d{1,5})(/executable)?$"
