...
import base
from metrics import load_routing, page_route_path as prp, models
...
class One(base.BaseCase):
    ...
    def test_save_pages_from_route_table(self):
        ...
        res = prp.save_pages(load_routing.routing_table())
    ...
    def test_save_pages_idempotency(self):
        ...
        res = prp.save_pages(load_routing.routing_table())
        prp.save_pages(load_routing.routing_table())
        prp.save_pages(load_routing.routing_table())
        self.assertEqual(models.Page.objects.count(), len(res))
        
    def test_all_examples(self):
        errors = []
        for route in load_routing.routing_table().values():
            try:
