...
def path_counts_query(table_id, from_date, to_date):
    ...
    new_query.update({
        'filters': ','.join([
            # we still receive hits for these urls, but they are all 404s
            # r'ga:pagePath=~^/content/elife/.*', # these paths appear in PDF files
            # r'ga:pagePath=~^/content/.*/e[0-9]{5}$', # these paths redirect to the most recent version
